...
export default function RootLayout() {
  ...
  return (
    <>
      <header
        ref={headerRef}
        className={`fixed top-0 z-50 w-full transform px-6 py-5 transition-all duration-500 ease-in-out ${
          isScrollingHeader ? "-translate-y-full" : "shadow-md bg-red-600"
        }`}>
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between">
          <div className="sm:mr-8">
          
            <span className="self-800 text-xl font-semibold text-teal-50 static">QuickDemand</span>
          </div>
          <nav
